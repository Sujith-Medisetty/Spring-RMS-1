...
public class showDetails2Excel {
	
	public static ByteArrayInputStream exportCustomerListToExcelFile(ArrayList<TellerPerformance> response) {
		...
		try {
			...
			String[] columnHeaders= {"Teller Id","Teller Name","Customers","Amount"};
			...
			ArrayList<TellerPerformance> a=response;
			System.out.println(a);
			int rownum=1;
			for(TellerPerformance i:a) {
				Row row=sh.createRow(rownum++);
				row.createCell(0).setCellValue(i.getTid());
				row.createCell(1).setCellValue(i.getTname());
				row.createCell(2).setCellValue(i.getCustomers());
